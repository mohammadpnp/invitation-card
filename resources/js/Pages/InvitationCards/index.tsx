...
export default function InvitationCards(): ReactElement {
	...
	useEffect(() => {
		...
		(async () => {
			await getInvitationCards(search_params).then((result): void => {
				setPlaceholder(false);
				
				setInvitationCards(result.items);
							...
							>
								...
								<CardActions>
									<Box sx={{display: 'flex', gap: theme.spacing(2), margin: 0}}>
										<Chip label={`گروه لبنیات`} component="a" clickable /><Chip label={`سالن ${hall} - غرفه ${booth}`}
										                                                            component="a"
										                                                            clickable />
									</Box>
